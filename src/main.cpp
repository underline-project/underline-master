...
#if defined(NDEBUG)
# error "Underline cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xff512008d74606a2ab2491d886b77b04bd92c773f4ed905d27de5fea5c987a82");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Underline: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Underline Signed Message:\n";
...
{
    // Underline: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Underline: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Underline: 0.5 days
static const int64 nTargetSpacing = 10 * 60; // Underline: 10 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Underline: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Underline: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x08ebbedbcb637490cac3b2028cedae35d87d8c41b00f1488bc996b1afd0003f4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "underline born 9 march";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1520588486;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086603631;

        if (fTestNet)
        {
            block.nTime    = 1520588464;
            block.nNonce   = 388304299;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd792639c3e0e741340eea83af54ef7f4583cce6dfc8eecc4228bf6dd4f1e87c5"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc1, 0xb7, 0xdc }; // Underline: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UnderlineMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UnderlineMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static UnderlineMiner(CWallet *pwallet)
{
    printf("UnderlineMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("underline-miner");
    ...
    try { loop {
        ...
        printf("Running UnderlineMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("UnderlineMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&UnderlineMiner, pwallet));
}
